...
This package also ships an HTTP entrypoint (`dist/http.js`, bin:
`mcp-freescout-http`) for hosted or containerized MCP access.

See [HOSTED.md](HOSTED.md) for:
...
Send a real reply to a FreeScout ticket, emailing the customer immediately. Unlike `freescout_create_draft_reply`, this is not reviewable before sending — it requires `confirm: true` as an explicit safety guard, and the call is rejected without it.

**Parameters:**

- `ticket` (required): Ticket ID, number, or FreeScout URL
- `replyText` (required): The reply content to send (generated by the LLM, supports Markdown formatting)
- `confirm` (required): Must be `true`. This sends a real email to the customer and cannot be undone.
- `userId` (optional): User ID sending the reply (defaults to env setting)
- `to` (optional): List of TO recipients. Omit to preserve existing recipients; pass `[]` to clear.
- `cc` (optional): List of CC recipients. Omit to preserve existing recipients; pass `[]` to clear.
- `bcc` (optional): List of BCC recipients. Omit to preserve existing recipients; pass `[]` to clear.
...
- "Send this reply to the customer on ticket #12345"
- "Reply to ticket 34811 confirming the fix is deployed"
